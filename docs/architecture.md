...
表示モードで保護する対象はモチーフ画像です。ふわふわモードはモチーフをローカルらくがきへ置換し、モザイクモードはモチーフをぼかします。どちらもモチーフの拡大とダウンロードを無効化し、通常モードだけが許可します。デザインボードなどの制作素材は、どの表示モードでも鮮明に表示し、拡大・ダウンロードできます。大きなヒーローモチーフは3モードを個別指定でき、指定がなければ全体モードに従います。
