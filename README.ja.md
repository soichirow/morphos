...
| モード   | モチーフ画像                     | モチーフ画像のダウンロード |
| -------- | -------------------------------- | -------------------------- |
| ふわふわ | ローカルの抽象化したらくがき画像 | 非表示                     |
| モザイク | 元画像を強くぼかし、格子を重ねる | 非表示                     |
| 通常     | 原作の通常画像                   | 表示                       |

初期値は「ふわふわ」です。選択はlocalStorageへ保存されます。詳細な安全境界は [利用ガイド](docs/user-guide.md#表示モード) に記載しています。

選択中の大きなモチーフは、全体モードを変えずに個別の表示モードを指定できます。個別指定もブラウザへ保存されます。

デザインボードなどの制作素材は、表示モードにかかわらず鮮明に表示し、拡大・ダウンロードできます。
